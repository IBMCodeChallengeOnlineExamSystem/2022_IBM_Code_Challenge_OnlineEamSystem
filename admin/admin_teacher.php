...
include '../connect.php';
   // $id= $_SESSION['ID'];
        $rs = mysqli_query($con,"SELECT * FROM login WHERE status='0' and type1='teacher' " );
        echo '<h2 style="color: aliceblue" align ="center" >Manage Teachers</h2>';
        echo"<table border='2px' align=center style='color: cornsilk'><th>name</th><th>College</th><th>Qualification</th><th>Experiance</th><th colspan = 2>Action</th>";
   while($row1=mysqli_fetch_assoc($rs))
   {
        $logid=$row1['loginid'];
    
        $cs = mysqli_query($con,"SELECT * FROM teacher WHERE loginid='$logid' " );
